    ...
    p1600: [
      {
        ...
        moves:[
          {from:'d5',to:'c7',label:'Nc7+!'},{from:'e8',to:'d7',label:'Kd7'},
          {from:'c7',to:'a8',label:'Nxa8'}
        ],
        steps:[
          {tip:'White to move. The Black king on e8 and the rook on a8 can both be attacked by the knight from a single square. Find the fork.',hl:[[3,3,'green'],[0,4,'blue'],[0,0,'blue']]},
          {tip:'1.Nc7+! The classic royal fork — the knight attacks the king on e8 AND the rook on a8 simultaneously, with check. Black must deal with the check first.',hl:[[1,2,'green']],arrows:[[3,3,1,2,'rgba(201,168,76,.7)'],[1,2,0,4,'rgba(201,168,76,.5)'],[1,2,0,0,'rgba(201,168,76,.5)']]},
          {tip:'1...Kd7 — The king steps aside, but the rook is doomed.',hl:[[1,3,'blue']]},
          {tip:'2.Nxa8 — White wins the rook. Even if the knight is later trapped in the corner, rook-for-knight is a winning material advantage. Key habit: whenever your knight can jump near the enemy king, scan for fork squares.',hl:[[0,0,'green']],warn:'After 2...Kc7 trying to trap the knight, White plays 3.Nb6! and even if Black captures with Kxb6, White has traded a knight (3 points) for a rook (5 points) — a clean win of the exchange.'}
        ]
      },
      {
        ...
        moves:[
          {from:'d1',to:'d8',label:'Qxd8#'}
        ],
        steps:[
          {tip:'White to move. Black\'s king is sealed in by its own pawns on f7, g7 and h7, and only the rook on d8 guards the back rank. Find the mate in one.',hl:[[0,3,'blue'],[0,6,'blue']]},
          {tip:'1.Qxd8# — CHECKMATE! The queen captures the rook, and the Black king has nowhere to go: f8 and h8 are covered along the rank, and f7, g7, h7 are blocked by Black\'s own pawns. The back rank is the most common mating weakness at club level.',hl:[[0,3,'green']],arrows:[[7,3,0,3,'rgba(201,168,76,.7)']],warn:'Prevention: give your king an escape square (luft) by playing h6 or g6 once your rooks leave the back rank. One quiet pawn move avoids hundreds of back-rank disasters.'}
        ]
      },
      {
        title:'Exploiting the Pin',
        fen:'r2qk2r/p4ppp/2n5/1B6/8/8/PPP2PPP/R1BQK2R w KQkq - 0 1',
        moves:[
          {from:'b5',to:'c6',label:'Bxc6+!'}
        ],
        steps:[
          {tip:'White to move. The bishop on b5 pins the knight on c6 against the Black king — the knight cannot legally move. Notice that nothing can recapture on c6. How does White win material?',hl:[[3,1,'green'],[2,2,'blue']],arrows:[[3,1,2,2,'rgba(201,168,76,.6)'],[2,2,0,4,'rgba(201,168,76,.3)']]},
          {tip:'1.Bxc6+! White simply captures the pinned knight — with check! No Black pawn can recapture (the b7 pawn is gone), so White has won a full piece. A pinned piece is frozen: it cannot run, and here it could not even be defended.',hl:[[2,2,'green']],warn:'Key principle: a piece pinned to the king is paralysed. Before moving anything else, always check whether you can simply capture a pinned piece — or pile more attackers onto it.'}
        ]
      },
      {
        title:'The Skewer',
        ...
        steps:[
          {tip:'White to move. King and queen stand on the same file — with the queen IN FRONT. This is the geometry of a skewer: capture the front piece, and the piece behind cannot help. What does White play?',hl:[[7,1,'green'],[3,1,'blue'],[0,1,'blue']],arrows:[[7,1,0,1,'rgba(201,168,76,.35)']]},
          {tip:'1.Rxb5+! The rook captures the queen — with check, because king and queen were lined up on the b-file. This is the skewer in its purest form: the valuable piece in front falls, and the piece behind it could not defend.',hl:[[3,1,'green']],arrows:[[7,1,3,1,'rgba(201,168,76,.7)']]},
          {tip:'1...Kc7 — The king steps out of check, but the damage is done: White has won a whole queen for nothing. Train your eye to spot enemy pieces standing on the same line — file, rank or diagonal — as their own king.',hl:[[1,2,'blue']],warn:'Skewer vs pin: in a PIN the valuable piece is BEHIND (so the front piece is frozen); in a SKEWER the valuable piece is IN FRONT (so it must move and abandon what stands behind it).'}
        ]
      },
      {
        ...
        steps:[
          {tip:'White to move. The knight on d6 stands on the same file as the White rook and the Black king. What happens if the knight moves off that file — to a square that also attacks something valuable?',hl:[[2,3,'green'],[7,3,'green'],[0,3,'blue']],arrows:[[7,3,0,3,'rgba(201,168,76,.3)']]},
          {tip:'1.Nf7+! DISCOVERED CHECK — the knight steps off the d-file, unveiling the rook\'s attack on the king, while the knight itself attacks the queen on h8. Two threats with one move; Black can only answer the check.',hl:[[1,5,'green']],arrows:[[7,3,0,3,'rgba(201,168,76,.7)'],[1,5,0,7,'rgba(201,168,76,.5)']]},
          {tip:'1...Kc8 — The king must escape the rook\'s check, abandoning the queen.',hl:[[0,2,'blue']]},
          {tip:'2.Nxh8 — The knight captures the queen. White has won a queen for nothing. Discovered checks are effectively two moves in one: the moving piece makes a threat while the unveiled piece gives check.',hl:[[0,7,'green']],warn:'Train yourself to see line-ups: your rook, bishop or queen standing behind one of your own pieces, aimed at the enemy king. Every such battery is a potential discovered attack — and the moving piece can go anywhere, even to squares that look absurd.'}
        ]
      }
    ],
    p1900: [
      {
        ...
        fen:'r1bq1rk1/ppp2ppp/2n1p3/3p4/3P4/3B1N2/PPP2PPP/R1BQ1RK1 w - - 0 1',
        moves:[
          {from:'d3',to:'h7',label:'Bxh7+!'},{from:'g8',to:'h7',label:'Kxh7'},
          {from:'f3',to:'g5',label:'Ng5+'},{from:'h7',to:'g8',label:'Kg8'},
          {from:'d1',to:'h5',label:'Qh5'},{from:'f8',to:'e8',label:'Re8'},
          {from:'h5',to:'h7',label:'Qh7+'},{from:'g8',to:'f8',label:'Kf8'},
          {from:'h7',to:'h8',label:'Qh8+'},{from:'f8',to:'e7',label:'Ke7'},
          {from:'h8',to:'g7',label:'Qxg7'}
        ],
        steps:[
          {tip:'White to move. All three Greek Gift conditions are met: bishop on d3 aimed at h7, knight on f3 ready for g5, queen ready for h5 — and no Black knight on f6 to defend. Begin the classic sacrifice.',hl:[[5,3,'green']],arrows:[[5,3,1,7,'rgba(201,168,76,.5)']]},
          {tip:'1.Bxh7+! The Greek Gift. White gives up the bishop to rip open the king\'s shelter.',hl:[[1,7,'green']],arrows:[[5,3,1,7,'rgba(201,168,76,.7)']]},
          {tip:'1...Kxh7 — Declining with 1...Kh8 leaves White simply a pawn up with an attack.',hl:[[1,7,'blue']]},
          {tip:'2.Ng5+ — The knight joins with check. This is the move that makes the sacrifice work — the queen is coming to h5 next.',hl:[[3,6,'green']],arrows:[[5,5,3,6,'rgba(201,168,76,.7)']],warn:'If 2...Kg6 the king walks into 3.Qg4! with a crushing attack. If 2...Kh6?? 3.Nxf7+ discovered check forks king and queen. The main defensive try is 2...Kg8.'},
          {tip:'2...Kg8 — Back to base. Now comes the queen.',hl:[[0,6,'blue']]},
          {tip:'3.Qh5! — Threatening Qh7 checkmate. Black\'s defences are stretched to breaking point.',hl:[[3,7,'green']],arrows:[[7,3,3,7,'rgba(201,168,76,.7)'],[3,7,1,7,'rgba(201,168,76,.4)']],warn:'Black\'s most stubborn defence is 3...Qxg5! 4.Bxg5 — giving up the queen for two minor pieces. White emerges with a decisive material advantage either way.'},
          {tip:'3...Re8 — Trying to make luft via f8, but it is too slow.',hl:[[0,4,'blue']]},
          {tip:'4.Qh7+ — The king is driven onto the back rank.',hl:[[1,7,'green']],arrows:[[3,7,1,7,'rgba(201,168,76,.7)']]},
          {tip:'4...Kf8 — Forced.',hl:[[0,5,'blue']]},
          {tip:'5.Qh8+ — The queen chases the king into the open.',hl:[[0,7,'green']]},
          {tip:'5...Ke7 — The king runs, but the position has collapsed.',hl:[[1,4,'blue']]},
          {tip:'6.Qxg7 — White regains material with a continuing attack against the exposed king — the g7 and f7 pawns fall, and Black\'s king will never find safety. The Greek Gift: memorise the conditions (Bd3, Nf3, Qd1, no Nf6) and it will win you many games.',hl:[[1,6,'green']],warn:'Checklist before sacrificing: (1) bishop hits h7, (2) Ng5+ is available, (3) queen can reach h5 quickly, (4) NO Black knight on f6 and no easy ...Nf6 defence. Missing any condition usually refutes the sacrifice.'}
        ]
      },
      {
        title:'Pawn Fork: Double Attack',
        fen:'r2qkb1r/pppp1ppp/2n1bn2/8/3P4/2N2N2/PPP1PPPP/R1BQKB1R w KQkq - 0 1',
        moves:[
          {from:'d4',to:'d5',label:'d5!'},{from:'e6',to:'d5',label:'Bxd5'},
          {from:'c3',to:'d5',label:'Nxd5'},{from:'f6',to:'d5',label:'Nxd5'},
          {from:'d1',to:'d5',label:'Qxd5'}
        ],
        steps:[
          {tip:'White to move. Black has carelessly placed a knight on c6 and a bishop on e6 — both on squares a White pawn can attack at once. Find the double attack.',hl:[[2,2,'blue'],[2,4,'blue'],[4,3,'green']]},
          {tip:'1.d5! The pawn fork. One humble pawn attacks both the knight on c6 and the bishop on e6. Black can only save one piece — or enter a forced sequence that loses material anyway.',hl:[[3,3,'green']],arrows:[[4,3,3,3,'rgba(201,168,76,.7)'],[3,3,2,2,'rgba(201,168,76,.5)'],[3,3,2,4,'rgba(201,168,76,.5)']],warn:'If Black retreats with 1...Nb8 or 1...Ne5, White plays 2.dxe6 winning the bishop for the pawn. There is no way to save both pieces.'},
          {tip:'1...Bxd5 — Black takes the pawn, hoping the tactics work out. They do not: d5 is defended twice.',hl:[[3,3,'blue']]},
          {tip:'2.Nxd5 — First recapture.',hl:[[3,3,'green']],arrows:[[5,2,3,3,'rgba(201,168,76,.7)']]},
          {tip:'2...Nxd5 — Black trades again, but White\'s queen has the last word.',hl:[[3,3,'blue']]},
          {tip:'3.Qxd5 — The dust settles: White has won a bishop for a pawn (count it: White lost one pawn and gained bishop and knight while losing a knight). A full piece for a pawn is decisive. The pawn fork is the cheapest double attack in chess — always scan for two enemy pieces one pawn-push apart.',hl:[[3,3,'green']],arrows:[[7,3,3,3,'rgba(201,168,76,.7)']]}
        ]
      },
      {
        title:'Arabian Mate',
        fen:'5r1k/5ppp/5N2/8/8/8/8/7R w - - 0 1',
        moves:[
          {from:'h1',to:'h7',label:'Rxh7#'}
        ],
        steps:[
          {tip:'White to move. Black\'s king is cornered on h8, hemmed in by its own pawns, and White\'s knight on f6 controls the key escape squares g8 and h7. Find the mate in one.',hl:[[0,7,'blue'],[2,5,'green']],arrows:[[2,5,0,6,'rgba(201,168,76,.4)'],[2,5,1,7,'rgba(201,168,76,.4)']]},
          {tip:'1.Rxh7# — CHECKMATE! This is the Arabian Mate, one of the oldest recorded mating patterns. The rook captures on h7 with check; the king cannot take it because the knight on f6 defends h7, cannot go to g8 because the knight covers that too, and g7 is blocked by Black\'s own pawn.',hl:[[1,7,'green']],arrows:[[7,7,1,7,'rgba(201,168,76,.7)'],[2,5,1,7,'rgba(201,168,76,.5)'],[2,5,0,6,'rgba(201,168,76,.5)']],warn:'The Arabian Mate formula: enemy king in the corner + your knight on f6 (covering g8 and h7) + your rook reaching the h-file or back rank. The knight and rook coordinate perfectly — neither can deliver this mate alone.'}
        ]
      },
      {
        title:'Remove the Defender',
        fen:'r2q1rk1/ppp2ppp/5n2/6B1/8/8/PPQ2PPP/1B4K1 w - - 0 1',
        moves:[
          {from:'c2',to:'h7',label:'Qxh7+!!'},{from:'f6',to:'h7',label:'Nxh7'},
          {from:'g5',to:'d8',label:'Bxd8'},{from:'f8',to:'d8',label:'Rfxd8'}
        ],
        steps:[
          {tip:'White to move. The knight on f6 is doing two jobs at once: guarding h7 against the queen-bishop battery, AND blocking the g5-bishop\'s path to the queen on d8. An overloaded defender can be exploited by force. How?',hl:[[2,5,'blue'],[6,2,'green'],[7,1,'green']],arrows:[[7,1,1,7,'rgba(201,168,76,.35)'],[3,6,0,3,'rgba(201,168,76,.35)']]},
          {tip:'1.Qxh7+!! White crashes through anyway. The queen is protected by the bishop on b1, so the king cannot take — Black\'s ONLY legal move is Nxh7. The defender is forcibly removed from f6.',hl:[[1,7,'green']],arrows:[[6,2,1,7,'rgba(201,168,76,.7)'],[7,1,1,7,'rgba(201,168,76,.4)']]},
          {tip:'1...Nxh7 — Forced. And now the knight has abandoned its second duty: blocking the diagonal to d8.',hl:[[1,7,'blue']]},
          {tip:'2.Bxd8 — The bishop sweeps down the now-open diagonal and captures the queen. The combination is complete.',hl:[[0,3,'green']],arrows:[[3,6,0,3,'rgba(201,168,76,.7)']]},
          {tip:'2...Rfxd8 — Black recaptures, but count the material: White gave up a queen and gained a pawn, a knight and a queen — a clean knight and pawn ahead. The lesson: a piece defending two things at once defends neither. Force it to choose.',hl:[[0,3,'blue']],warn:'Overloading is everywhere at club level. Before every move, ask: which enemy piece is defending more than one thing? Can I force it to commit with a sacrifice or a threat?'}
        ]
      },
      {
        title:'Rook to the Seventh',
        fen:'r5k1/1pp3pp/p7/3p4/8/2P5/PP3PPP/4R1K1 w - - 0 1',
        moves:[
          {from:'e1',to:'e7',label:'Re7!'},{from:'g8',to:'f8',label:'Kf8'},
          {from:'e7',to:'c7',label:'Rxc7'}
        ],
        steps:[
          {tip:'White to move. The e-file is open and Black\'s seventh rank is undefended. Where does the rook belong?',hl:[[7,4,'green']]},
          {tip:'1.Re7! The rook invades the seventh rank, attacking the pawns on c7 and g7 simultaneously. A rook on the seventh is worth roughly an extra pawn in practical strength — it attacks everything and ties the enemy king down.',hl:[[1,4,'green']],arrows:[[7,4,1,4,'rgba(201,168,76,.7)'],[1,4,1,2,'rgba(201,168,76,.4)'],[1,4,1,6,'rgba(201,168,76,.4)']]},
          {tip:'1...Kf8 — Black attacks the rook and defends g7, but cannot save everything.',hl:[[0,5,'blue']]},
          {tip:'2.Rxc7 — The rook grabs the c7 pawn and stays dominant on the seventh; b7 falls next. From one rook lift, White has won a pawn and owns the rank. Rule of thumb: when a file opens, race your rook to the seventh rank before doing anything else.',hl:[[1,2,'green']],arrows:[[1,4,1,2,'rgba(201,168,76,.7)']],warn:'Two rooks on the seventh rank ("pigs on the seventh") are usually decisive — they devour pawns and create mating nets. One rook on the seventh is an advantage; two are a feast.'}
        ]
      }
    ],
    p2100: [
      {
        title:'King and Pawn: Escort to Promotion',
        fen:'3k4/8/3K4/3P4/8/8/8/8 w - - 0 1',
        moves:[
          {from:'d6',to:'e6',label:'Ke6!'},{from:'d8',to:'e8',label:'Ke8'},
          {from:'d5',to:'d6',label:'d6'},{from:'e8',to:'d8',label:'Kd8'},
          {from:'d6',to:'d7',label:'d7'},{from:'d8',to:'c7',label:'Kc7'},
          {from:'e6',to:'e7',label:'Ke7'},{from:'c7',to:'c6',label:'Kc6'},
          {from:'d7',to:'d8',label:'d8=Q',promo:'Q'}
        ],
        steps:[
          {tip:'White to move. The White king stands on the 6th rank IN FRONT of its pawn — the textbook winning configuration. This position is won no matter whose move it is. Demonstrate the technique.',hl:[[2,3,'green'],[3,3,'green'],[0,3,'blue']]},
          {tip:'1.Ke6! The king steps aside and FORWARD, taking control of the promotion path. The pawn stays back — the king leads, the pawn follows. (1.Kc6 works symmetrically.)',hl:[[2,4,'green']],arrows:[[2,3,2,4,'rgba(201,168,76,.7)']],warn:'The golden rule of king-and-pawn endings: when your king reaches the 6th rank in front of its pawn, the win is automatic. Without that, a lone pawn often only draws — the defending king blockades it.'},
          {tip:'1...Ke8 — Black keeps blocking the path as best he can.',hl:[[0,4,'blue']]},
          {tip:'2.d6 — Now the pawn advances under the king\'s protection.',hl:[[2,3,'green']],arrows:[[3,3,2,3,'rgba(201,168,76,.7)']]},
          {tip:'2...Kd8 — Back in front of the pawn — but White\'s king controls all the key squares.',hl:[[0,3,'blue']]},
          {tip:'3.d7 — The pawn reaches the seventh. Note it gives no check — that is exactly what White wants. (A pawn arriving on the 7th WITH check is often only a draw!)',hl:[[1,3,'green']],arrows:[[2,3,1,3,'rgba(201,168,76,.7)']]},
          {tip:'3...Kc7 — The king must step aside; d8 was no longer available... and Kxd7 is impossible because White\'s king guards the pawn.',hl:[[1,2,'blue']]},
          {tip:'4.Ke7 — The White king seizes the promotion square\'s neighbourhood. Nothing can stop d8=Q now.',hl:[[1,4,'green']],arrows:[[2,4,1,4,'rgba(201,168,76,.7)']]},
          {tip:'4...Kc6 — A last shuffle.',hl:[[2,2,'blue']]},
          {tip:'5.d8=Q — Promotion. The technique to memorise: KING IN FRONT, PAWN BEHIND. The single most important endgame pattern in chess — it converts hundreds of equal-looking endings into wins.',hl:[[0,3,'green']],arrows:[[1,3,0,3,'rgba(201,168,76,.7)']]}
        ]
      },
      {
        title:'Smothered Mate (Philidor\'s Legacy)',
        fen:'r4r1k/6pp/8/6N1/8/1Q6/6PP/7K w - - 0 1',
        moves:[
          {from:'g5',to:'f7',label:'Nf7+'},{from:'h8',to:'g8',label:'Kg8'},
          {from:'f7',to:'h6',label:'Nh6+'},{from:'g8',to:'h8',label:'Kh8'},
          {from:'b3',to:'g8',label:'Qg8+!!'},{from:'f8',to:'g8',label:'Rxg8'},
          {from:'h6',to:'f7',label:'Nf7#'}
        ],
        steps:[
          {tip:'White to move. Queen on the b3-g8 diagonal, knight on g5, Black king stuck in the corner: every ingredient of the most famous combination in chess is present. Begin.',hl:[[5,1,'green'],[3,6,'green'],[0,7,'blue']],arrows:[[5,1,0,6,'rgba(201,168,76,.35)']]},
          {tip:'1.Nf7+ — The knight checks from f7.',hl:[[1,5,'green']],arrows:[[3,6,1,5,'rgba(201,168,76,.7)']],warn:'If Black tries 1...Rxf7 instead, then 2.Qxf7 wins the exchange with a continuing attack — also decisive for White.'},
          {tip:'1...Kg8 — The only square (f8 is occupied by Black\'s own rook).',hl:[[0,6,'blue']]},
          {tip:'2.Nh6+ — DOUBLE CHECK! The knight checks from h6 AND discovers the queen\'s check along the diagonal. Against double check, capturing or blocking is impossible — the king MUST move.',hl:[[2,7,'green']],arrows:[[1,5,2,7,'rgba(201,168,76,.7)'],[5,1,0,6,'rgba(201,168,76,.5)']]},
          {tip:'2...Kh8 — Forced back into the corner (f8 still blocked by the rook).',hl:[[0,7,'blue']]},
          {tip:'3.Qg8+!! The immortal queen sacrifice. The king cannot capture — the knight on h6 guards g8. Black\'s only legal move is to take with the rook.',hl:[[0,6,'green']],arrows:[[5,1,0,6,'rgba(201,168,76,.7)']]},
          {tip:'3...Rxg8 — Forced. And now the king is completely smothered by its own pieces: rook on g8, pawns on g7 and h7.',hl:[[0,6,'blue']]},
          {tip:'4.Nf7# — SMOTHERED MATE. The knight returns to f7 and the king, buried alive by its own army, has no square. Philidor\'s Legacy — the queen sacrifice forces Black to build his own tomb. Memorise the sequence: Nf7+, Nh6+ (double check), Qg8+!!, Nf7#.',hl:[[1,5,'green']],arrows:[[2,7,1,5,'rgba(201,168,76,.7)']],warn:'The pattern requires: enemy king on g8/h8, your knight reaching f7 and h6, your queen able to reach g8, and the f8 escape covered (here by Black\'s own rook). When you see a cramped castled king, start calculating this sequence.'}
        ]
      },
      {
        title:'Underpromotion: The Rook Wins',
        fen:'8/5P1k/5K2/8/8/8/8/8 w - - 0 1',
        moves:[
          {from:'f7',to:'f8',label:'f8=R!',promo:'R'},{from:'h7',to:'h6',label:'Kh6'},
          {from:'f8',to:'h8',label:'Rh8#'}
        ],
        steps:[
          {tip:'White to move. The pawn is one step from promotion. But careful — calculate what happens after the "automatic" 1.f8=Q before you play it. Why is the obvious move a catastrophe?',hl:[[1,5,'green'],[1,7,'blue']],warn:'1.f8=Q?? is STALEMATE — a draw! Check it: the Black king on h7 would have no legal move (g8, h8, g6, g7 and h6 are all covered by the new queen and the White king) and is not in check. Promoting to the strongest piece throws away the win.'},
          {tip:'1.f8=R! Underpromotion to a ROOK. Unlike the queen, the rook does not cover h6 — Black keeps exactly one legal move, so there is no stalemate, and the mating net closes anyway.',hl:[[0,5,'green']],arrows:[[1,5,0,5,'rgba(201,168,76,.7)']]},
          {tip:'1...Kh6 — The only legal move, and it walks into mate.',hl:[[2,7,'blue']]},
          {tip:'2.Rh8# — CHECKMATE. The rook checks down the h-file, the White king covers g5, g6 and g7, and there is nowhere to run. The lesson: promotion is a CHOICE. In about 1 position in 1000, queen is wrong — and those positions decide games. Always glance at stalemate before promoting.',hl:[[0,7,'green']],arrows:[[0,5,0,7,'rgba(201,168,76,.7)'],[0,7,2,7,'rgba(201,168,76,.5)']]}
        ]
      },
      {
        title:'Remove the Guard, Then Promote',
        fen:'1r4k1/P5pp/8/8/8/8/6PP/1R4K1 w - - 0 1',
        moves:[
          {from:'b1',to:'b8',label:'Rxb8+!'},{from:'g8',to:'f7',label:'Kf7'},
          {from:'a7',to:'a8',label:'a8=Q',promo:'Q'}
        ],
        steps:[
          {tip:'White to move. The a7 pawn is one square from queening, but the Black rook on b8 guards the promotion square a8 (and Black threatens ...Rxa8 ideas forever). How does White settle the matter immediately?',hl:[[1,0,'green'],[0,1,'blue'],[0,0,'blue']],arrows:[[1,0,0,0,'rgba(201,168,76,.4)']]},
          {tip:'1.Rxb8+! Eliminate the guard — with check! The Black rook was the only piece controlling a8, and White\'s rook removes it by force. Because it comes with check, Black has no time for anything.',hl:[[0,1,'green']],arrows:[[7,1,0,1,'rgba(201,168,76,.7)']]},
          {tip:'1...Kf7 — The king must answer the check (g7 and h7 are blocked by Black\'s own pawns).',hl:[[1,5,'blue']]},
          {tip:'2.a8=Q — Promotion, and White is a whole rook and queen ahead. The deflection theme in its simplest form: identify the ONE piece preventing your plan, and ask whether you can capture or chase it — especially with check, which gives the opponent no time to respond.',hl:[[0,0,'green']],arrows:[[1,0,0,0,'rgba(201,168,76,.7)']],warn:'Passed pawns on the 7th rank are worth material sacrifices. A rook given up to force promotion usually returns with interest — here White invested nothing at all, since Rxb8 captured a rook of equal value first.'}
        ]
      },
      {
        title:'Anastasia\'s Mate',
        fen:'r4rk1/5ppp/8/3N4/4Q3/R7/5PPP/6K1 w - - 0 1',
        moves:[
          {from:'d5',to:'e7',label:'Ne7+'},{from:'g8',to:'h8',label:'Kh8'},
          {from:'e4',to:'h7',label:'Qxh7+!!'},{from:'h8',to:'h7',label:'Kxh7'},
          {from:'a3',to:'h3',label:'Rh3#'}
        ],
        steps:[
          {tip:'White to move. The knight can reach e7 — a square from which it controls BOTH g8 and g6, the king\'s flight squares on the g-file. Combined with a rook lift to the h-file, this is the recipe for Anastasia\'s Mate. Begin the combination.',hl:[[3,3,'green'],[4,4,'green'],[5,0,'green']]},
          {tip:'1.Ne7+ — The knight checks and takes control of g8 and g6. The cage is being built.',hl:[[1,4,'green']],arrows:[[3,3,1,4,'rgba(201,168,76,.7)'],[1,4,0,6,'rgba(201,168,76,.4)']]},
          {tip:'1...Kh8 — The only move; f8 is occupied by Black\'s own rook.',hl:[[0,7,'blue']]},
          {tip:'2.Qxh7+!! The queen sacrifice rips the h-file open. The king must capture — there is no other legal response to the check.',hl:[[1,7,'green']],arrows:[[4,4,1,7,'rgba(201,168,76,.7)']]},
          {tip:'2...Kxh7 — Forced. The king now stands alone on the open h-file.',hl:[[1,7,'blue']]},
          {tip:'3.Rh3# — ANASTASIA\'S MATE. The rook swings across to the h-file with checkmate: g6 and g8 are controlled by the knight, g7 is blocked by Black\'s own pawn, and the h-file belongs to the rook. The pattern: knight on e7 + open h-file + rook lift = mate. It appears constantly in real games — learn to spot a knight that can reach e7 (or e2 for Black) near a castled king.',hl:[[5,7,'green']],arrows:[[5,0,5,7,'rgba(201,168,76,.7)'],[1,4,0,6,'rgba(201,168,76,.4)']],warn:'Anastasia\'s Mate (named after an 1803 novel) needs three things: (1) knight on e7 controlling g8/g6, (2) enemy king drawn to the h-file — usually by a queen sacrifice on h7, (3) a rook able to reach the h-file. Whenever your knight lands on e7 with the enemy king castled, start calculating Qxh7+.'}
        ]
      }
    ]
  };
